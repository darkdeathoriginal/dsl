...
export const CodeBlock: React.FC<CodeBlockProps> = ({
  ...
  useEffect(() => {
    if (
      pyodide &&
      !originalStdout &&
      pyodide !== pyodideInstanceRefForStdHandles
    ) {
      ...
      console.log(
        "Capturing original Pyodide stdout/stderr handlers (if any)."
      );
  ...
  const runCode = useCallback(async () => {
    // useCallback to memoize if passed down
    ...
    pyodide.setStderr({
      batched: (msg) => setError((prev) => prev + msg + "\n"),
    });
...
`;
        ...
        if (result !== undefined && result !== null) {
          // Avoid printing 'None' or 'undefined'
      ...
      else if (originalStdout)
        pyodide.setStdout({
          batched: originalStdout,
        }); // Fallback to originally captured
      else
        pyodide.setStdout({
      ...
      else if (originalStderr)
        pyodide.setStderr({ batched: originalStderr }); // Fallback
      else
        pyodide.setStderr({
          batched: function (): void {
            throw new Error("Function not implemented.");
  ...
  if (pyodideLoading) {
    return (
      <Card>
        <CardContent>
          <p className="p-4 text-sm">Loading Python environment...</p>
        </CardContent>
      </Card>
    );
  }
  if (pyodideError) {
    return (
      <Alert variant="destructive" className="my-4">
        ...
        <AlertDescription>
          Could not load Python environment: {pyodideError.message}
        </AlertDescription>
      </Alert>
    );
  }
